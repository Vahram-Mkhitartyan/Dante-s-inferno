...
namespace Water2D
{
    ...
    public class Obstructor3D : MonoBehaviour
    {
        ...
        public void CreateData()
        {
...
#if UNITY_EDITOR
            if (ObstructorManager.GetInstance() != null && !ObstructorManager.GetInstance().obstructionObjectsVisible.value) UnityEditor.SceneVisibilityManager.instance.Hide(obstructor.gameObject, true);
#endif

            if (ObstructorManager.GetInstance() != null && !ObstructorManager.GetInstance().obstructionObjectsVisible.value) { obstructor.gameObject.hideFlags = HideFlags.HideInHierarchy; }
            ...
            obstructorMr.sharedMaterial = ObstructorManager.red3d;
